...
const GoogleAuthFallback = ({ onSuccess, onError }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result && result.redirect) {
        // Se for redirecionamento, não fazer nada (o usuário será redirecionado)
        return;
      }
      ...
      // Se for erro de COOP, tentar redirecionamento manual
      if (error.message && error.message.includes('Cross-Origin-Opener-Policy')) {
        console.log('🔄 Detectado erro COOP, usando redirecionamento manual');
        try {
